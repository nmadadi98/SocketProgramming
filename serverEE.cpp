...
int main(){
	...
	if ((bind(sockfd_UDP_EE, (sockaddr*) &my_addr, sizeof(my_addr)))<0){
		perror("bind failed");
	}

	cout<<"The ServerEE is up and running using UDP on port 23131."<<endl;
	...
	while(1){
		...
		recvfrom(sockfd_UDP_EE, buf,  MAXBUFLEN-1, 0, (struct sockaddr *)&servaddr_M, &servaddr_M_len);
		recvfrom(sockfd_UDP_EE, buf2, MAXBUFLEN-1, 0, (struct sockaddr *)&servaddr_M, &servaddr_M_len);
		...
		for (string x: vec_courses){
			...
			if (strcmp(c_code.c_str(),coursecode.c_str())==0) {
				...
				int s = sendto(sockfd_UDP_EE, information.c_str(), 1000, 0, (sockaddr*) &servaddr_M, sizeof(servaddr_M));
			...
			if (count_vec_size == 0){
				...
				sendto(sockfd_UDP_EE, information.c_str(), 1000, 0, (sockaddr*) &servaddr_M, sizeof(servaddr_M));
				cout << "The ServerEE finished sending the response to the Main Server." <<endl;
			}
